...
#ifdef DEBUG
				strncpy(logPath, szIni, sizeof(szIni));
				TCHAR* ptmp = _tcsrchr(logPath, _T('\\'));
				if (ptmp != NULL)
				{   //ファイル名を削除
...
// 最新メールID
static int LastMailID = 0;

// メールIDから最後のメール番号を抽出
int szMailtoID(char *szMailID)
{
	int  MailID = 0;

	TCHAR* ptmp = _tcsrchr(szMailID, _T('?'));
	log("ptmp =%s\n", ptmp);
	if (ptmp != NULL) {
		ptmp = _tcsinc(ptmp);   //一文字進める
		log("ptmp =%s\n", ptmp);
		MailID = strtol(ptmp, NULL, 16);
	}
	return MailID;
}

// iniファイルに保存
void SaveLastMail(char *szMailID)
{
	char szLastID[256];

	LastMailID = szMailtoID(szMailID);
	sprintf(szLastID, "%x", LastMailID);
	WritePrivateProfileString("Settings", "LastMailID", szLastID, szIni);
}

// プラグイン導入後一発目のiniファイル初期化
void InitLastMail()
{
	char szMailID[256];
	int n= -1;

	n = bka.GetNextMail(n, szMailID, 256, FALSE);
	log("szMailID =%s\n", szMailID);

	SaveLastMail(szMailID);
}

// 新着があれば振り分け
void FilterIfNewMail()
{
	char szMailID[256];
	int  MailID = 0;
	int n= -1;
	...
	MailID = szMailtoID(szMailID);
	log("MailID =%x\n", MailID);
	log("LastMailID =%x\n", LastMailID);

	if(LastMailID < MailID) {
		SaveLastMail(szMailID);
		bka.Command(NULL, "ManualFilter");
	}
}
...
int WINAPI BKC_OnStart()
{
	...
	*/
	char szLastID[256];
	GetPrivateProfileString("Settings", "LastMailID", "0", 
		&szLastID[0], sizeof(szLastID), szIni);
	LastMailID = strtol(szLastID, NULL, 16);
	log("LastMailID = %x\n", LastMailID);
...
int WINAPI BKC_OnOpenFolder(LPCTSTR lpFolderID)
{
	...
	// IMAPフォルダが対象
	if(strstr(lpFolderID, ".ini") != NULL) {
		name = bka.GetFolderDisplayName(lpFolderID);
		log("name =%s\n", name);
		log("curmail =%s\n", bka.GetCurrentMail());

		// 受信箱が対象
		if(strstr(name, "受信箱") != NULL) {
			if(LastMailID == 0) {
				// プラグイン起動後初回は最新のメール覚えるだけ
				InitLastMail();
			}
			else {
				// 新着メールがあれば振り分け
...
int WINAPI BKC_OnPlugInInfo(LPBKPLUGININFO lpPlugInInfo)
{
	...
	*/
	strcpy(lpPlugInInfo->szPlugInName, "Becky! IMAP Filter plug-in");
	strcpy(lpPlugInInfo->szVendor, "Y.Furukawa");
	strcpy(lpPlugInInfo->szVersion, "1.0");
	strcpy(lpPlugInInfo->szDescription, "Filter New-Mail on IMAP Inbox.");
